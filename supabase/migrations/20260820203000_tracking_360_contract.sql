-- Tracking 360 / Fase 2
-- Amplia o vocabulario comportamental e publica um contrato agregado, sem PII,
-- para a area autenticada de Inteligencia do ERP.
...
create index if not exists site_events_anon_campaign_idx
  on private.site_events_anon (utm_source, utm_campaign, occurred_at desc);

create index if not exists site_events_anon_item_idx
  on private.site_events_anon ((properties ->> 'item_id'), occurred_at desc)
  where properties ? 'item_id';
...
begin
  ...
  from funnel, event_totals, pages, campaigns, properties, consent, delivery, attribution;
